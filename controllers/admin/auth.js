...
exports.login = async (req, res, next) => {
    ...
    try {
        ...
        if (_.get(req.body, "email", false) && _.get(req.body, "password", false)) {
            ...
            if (!super_admin) {
                return res.status(422).send(utils.apiResponseMessage(false, "Unauthorized."));
            } else if (super_admin.status == false) {
                return res.status(422).send(utils.apiResponseMessage(false, "Your account is disable for now."))
            } else if (!super_admin.authenticate(req.body.password)) {
                return res.send(422).send(utils.apiResponseMessage(false, `Password is not valid.`))
            }
            ...
            return res.status(422).send(utils.apiResponseMessage(false, 'Invalid request.'));
        }
