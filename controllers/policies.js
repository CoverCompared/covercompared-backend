...
exports.storeMso = async (req, res, next) => {
  try {
    ...
    if (!validation) {
      res
        ...
        .send(
          utils.apiResponse(
            false,
            utils.getErrorMessage(v.errors),
            {},
            v.errors
          )
        );
    ...
    if (
      utils.getFormattedAmount(parseFloat(req.body.amount)) !=
      utils.getFormattedAmount(
        parseFloat(req.body.quote) + parseFloat(req.body.mso_addon_service)
      )
    ) {
      return res
        .status(200)
        .send(
          utils.apiResponseMessage(
            false,
            "Amount does not match of Quote and Addon Service"
          )
        );
    }
    ...
    if (
      ...
      return res
        .status(200)
        .send(utils.apiResponseMessage(false, "Total Amount is invalid."));
    }
    ...
    return res.status(200).send(
      utils.apiResponse(true, "Policy added successfully.", {
        _id: policy._id,
...
exports.msoConfirmPayment = async (req, res, next) => {
  try {
    ...
    if (
      ...
      policy.MSOPolicy.contract_product_id
    ) {
      return res.status(200).send(
    ...
    // Create Payment
    payment =
      payment && payment.payment_hash == req.body.payment_hash
        ? payment
        : new Payments();
    ...
    await web3Connect.smart_contracts.mso.msoSyncTransactionForApi(
      req.body.payment_hash
    );
...
exports.deviceConfirmPayment = async (req, res, next) => {
  try {
    ...
    await web3Connect.smart_contracts.p4l.p4lSyncTransactionForApi(
      req.body.payment_hash
    );
...
exports.policyReview = async (req, res, next) => {
  try {
    ...
    if (!policy) {
      ...
      ![constant.PolicyStatus.active, constant.PolicyStatus.complete].includes(
        policy.status
      )
    ) {
      return res
        .status(200)
        .send(
          utils.apiResponseMessage(false, "Policy status is not activated.")
        );
    }
    ...
    return res.status(200).send(
      utils.apiResponse(true, "Review added successfully.", {
        _id: review._id,
      })
    );
  } catch (error) {
  ...
  try {
    ...
    for (const key in policies) {
      ...
      if (policy.product_type == constant.ProductTypes.smart_contract) {
        policy.logo = await companies.getCoverImage(
          _.get(policy, "details.unique_id", "")
        );
      ...
      if (
        policy.payment &&
        Array.isArray(policy.payment) &&
        policy.payment.length
      ) {
        policy.payment = policy.payment[0];
        policy.payment.transaction_link = utils.getTransactionLink(
          policy.payment,
          policy
        );
        policy.payment.network_name = utils.getNetworkDetails(
          policy.payment,
          policy
        );
      } else {
...
exports.show = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .send(utils.apiResponseData(true, { ...policy[0], reviews }));
  } catch (error) {
    console.log("ERR", error);
    return res
      .status(500)
...
exports.storeSmartContract = async (req, res, next) => {
  ...
  try {
    ...
    await policy.save();
    let product_type = constant.CryptoExchangeTypes.includes(req.body.type)
      ? "Crypto Exchange"
      : "Smart Contract";

    return res.status(200).send(
      utils.apiResponse(true, `${product_type} added successfully.`, {
        _id: policy._id,
        txn_hash: policy.txn_hash,
      })
    );
...
exports.smartContractConfirmPayment = async (req, res, next) => {
  try {
    ...
    return res.status(200).send(
      utils.apiResponse(true, "Cover detail updated successfully.", {
        policy_id: policy._id,
        txn_hash: policy.txn_hash,
        payment_status: policy.payment_status,
        status: policy.status,
